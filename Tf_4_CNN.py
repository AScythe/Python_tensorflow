...
x_image = tf.reshape(xs, [-1, 28, 28, 1])# [number of input image, row of image, col of image, channel of image], for mnist dataset, image would be gray scale image ,so the channel should be 1
print(x_image.shape) 

# create 32 conv kernels which mean we will get 32 feature maps with the shape of 28 by 28
##  conv1 layer ##
...
##  func2 layer ##
W_fc2 = weight_variable([1024, 10]) # the number of final fully connected layer should be the same with the classes you have
b_fc2 = bias_variable([10])
prediction = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)

# We choose cross entropy as our loss function, normally it should be coupled with softmax for solving classfication problem
...
with tf.Session() as sess:
    ...
    for i in range(1001):
        ...
        if i %50 ==0:
            print("Training step: {}, accuracy rate: {}%".format(i, 100*compute_accuracy(
        ...
        #     print('type(batch_ys:)',type(batch_ys))# numpy.ndarray
            ...
            print('prediction class: ', np.argmax(sess.run(prediction,feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 0.5})))
            picture = np.reshape(batch_xs, (28,28))
            plt.figure()
...
"""
# Show image in the mnist dataset
